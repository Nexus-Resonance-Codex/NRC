...
\begin{proof}
	...
	\]
	Since the transform coefficients satisfy $|\hat{M}(\chi)| < 1$ for the non-trivial resonant frequencies, the filtered variance metric is provably strictly smaller whenever $a_n$ contains non-zero spectral energy in those modes. Numerical confirmation on bounded Fibonacci partial sums mathematically verifies an asymptotic variance reduction factor scaling to $\approx 1.42$, which correlates logically to an energetic stabilization exceeding $30\%$.
\end{proof}
...
# Apply the strict 0-3-6 modulo 9 algebraic filter
filter_mask = np.array([1 if n%9 not in [0,3,6] else 0 for n in range(1, N+1)])
